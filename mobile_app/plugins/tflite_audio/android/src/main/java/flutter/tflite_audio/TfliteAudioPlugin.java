...
public class TfliteAudioPlugin implements MethodCallHandler, StreamHandler, FlutterPlugin, ActivityAware,
    ...
    private boolean determineAudio(){
        ...
        if(isAudioInput){ 
            ...
            boolean result = inputShape.length > 1 ? inputShape[0] > inputShape[1] : false;
    ...
    private void loadModel(){
        ...
        try {
            if (isAsset) {
                key = FlutterInjector.instance().flutterLoader().getLookupKeyForAsset(modelPath);
        ...
        if (labelPath.length() > 0) {
            if (isAsset) {
                key = FlutterInjector.instance().flutterLoader().getLookupKeyForAsset(labelPath);
    ...
    private void loadAudioFile() {
        ...
        try {
            if (isAsset) {
                ...
                String key = FlutterInjector.instance().flutterLoader().getLookupKeyForAsset(audioDirectory);
    ...
    private void startRecognition(short[] inputBuffer16) {
        ...
        Object tfOutput = outputShape.length == 1 ? outputTensor[0] : outputTensor;
        ...
        switch (inputType) {
            ...
            case "mfcc":
                ...
                tfLite.run(inputData2D, tfOutput);
                break;
            ...
            case "melSpectrogram":
                ...
                tfLite.run(inputData4D, tfOutput);
                break;
            ...
            case "rawAudio":
                ...
                if (inputShape.length == 1) {
                    tfLite.runForMultipleInputsOutputs(new Object[]{inputData2D[0]}, outputMap);
                } else {
                    tfLite.runForMultipleInputsOutputs(new Object[]{inputData2D}, outputMap);
                }
                break;
