...
public class LabelSmoothing {
    ...
    public RecognitionResult processLatestResults(float[] currentResults, long currentTimeMS) {
        if (currentResults.length != labelsCount) {
            Log.w("LabelSmoothing", "Label count (" + labelsCount + ") does not match model output size (" + currentResults.length + ")");
            labelsCount = Math.min(currentResults.length, labelsCount);
        }
        ...
        for (Pair<Long, float[]> previousResult : previousResults) {
            ...
            int i = 0;
            while (i < labelsCount && i < scoresTensor.length) {
